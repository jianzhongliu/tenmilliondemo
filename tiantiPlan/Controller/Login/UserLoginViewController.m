...
- (UIButton *)buttonPhone {
    if (_buttonPhone == nil) {
        ...
        [_buttonPhone setTitleColor:DSGrayColor6 forState:UIControlStateNormal];
...
- (void)loginWX {
    [self sendUserActionLog:@"login_wx"];    
    ...
    snsPlatform.loginClickHandler(self,[UMSocialControllerService defaultControllerService],YES,^(UMSocialResponseEntity *response){
        ...
        if (dicresult && dicresult.count > 0) {
            [self sendUserActionLog:@"login_wx_success"];
...
- (void)loginSina {
    ...
    [self sendUserActionLog:@"login_sina"];
    UMSocialSnsPlatform *snsPlatform = [UMSocialSnsPlatformManager getSocialPlatformWithName:UMShareToSina];

    snsPlatform.loginClickHandler(self,[UMSocialControllerService defaultControllerService],YES,^(UMSocialResponseEntity *response){
        NSDictionary *dicresult = [response.data objectForKey:@"sina"];
        if (dicresult && dicresult.count > 0) {
            [self sendUserActionLog:@"login_sina_success"];
...
- (void)requestThirdPartLoginWith:(NSDictionary *) dicParam {
    ...
    NSString *url = [NSString stringWithFormat:@"%@lovers/dologin",DPHOST];
    NSDictionary *dic = @{@"identify":dicParam[@"usid"]};
    
    [manager GET:url parameters:dic success:^(AFHTTPRequestOperation *operation, id responseObject) {
        if ([responseObject isKindOfClass:[NSDictionary class]]) {
            NSDictionary *dic = (NSDictionary *)responseObject;
            NSDictionary *userDic = [dic objectForKey:@"userModel"];
...
//            [UserCacheBean share].userModel = user;
//            [UserCacheBean fetchTouristInfo];
